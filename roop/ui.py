...
"""
    ...
    while run_server:
        ...
        with gr.Blocks(title=f'{roop.metadata.name} {roop.metadata.version}', theme=roop.globals.CFG.selected_theme, css=mycss) as ui:
            ...
            with gr.Tab("Intercambio de Caras"):
                with gr.Row():
                    with gr.Column():
                        input_faces = gr.Gallery(label="Caras de origen", allow_preview=True, preview=True, height=128, object_fit="scale-down")
                        with gr.Row():
                                bt_remove_selected_input_face = gr.Button("Eliminar seleccionada")
                                bt_clear_input_faces = gr.Button("Limpiar todo", variant='stop')
                        bt_srcimg = gr.Image(label='Imagen de Cara Origen', type='filepath', tool=None)
                    with gr.Column():
                        target_faces = gr.Gallery(label="Caras objetivo", allow_preview=True, preview=True, height=128, object_fit="scale-down")
                        with gr.Row():
                                bt_remove_selected_target_face = gr.Button("Eliminar seleccionada")
                        bt_destfiles = gr.Files(label='Archivo(s) Objetivo', file_count="multiple", elem_id='filelist')
                with gr.Row():
                    with gr.Column(visible=False) as dynamic_face_selection:
                        face_selection = gr.Gallery(label="Caras detectadas", allow_preview=True, preview=True, height=256, object_fit="scale-down")
                        with gr.Row():
                            bt_faceselect = gr.Button("Usar cara seleccionada")
                            bt_cancelfaceselect = gr.Button("Listo")
            
                with gr.Row():
                    with gr.Column(scale=1):
                        selected_face_detection = gr.Dropdown(["Primera encontrada", "Todas las caras", "Cara seleccionada", "Todas las mujeres", "Todos los hombres"], value="Primera encontrada", label="Seleccionar detección de caras para intercambio")
                        max_face_distance = gr.Slider(0.01, 1.0, value=0.65, label="Umbral Máximo de Similitud Facial")
                    with gr.Column(scale=1):
                        video_swapping_method = gr.Dropdown(["Extraer Frames a media","Procesamiento en Memoria"], value="Procesamiento en Memoria", label="Seleccionar método de procesamiento de video", interactive=True)
                        chk_det_size = gr.Checkbox(label="Usar Det-Size por defecto", value=True, elem_id='default_det_size', interactive=True)
                    with gr.Column(scale=2):
                        roop.globals.keep_fps = gr.Checkbox(label="Mantener FPS", value=True)
                        roop.globals.keep_frames = gr.Checkbox(label="Mantener Frames (relevante solo al extraer frames)", value=False)
                        roop.globals.skip_audio = gr.Checkbox(label="Omitir audio", value=False)
                with gr.Row():
                    with gr.Column():
                        selected_enhancer = gr.Dropdown(["Ninguno", "Codeformer", "DMDNet", "GFPGAN"], value="Ninguno", label="Seleccionar post-procesamiento")
                    with gr.Column():
                        blend_ratio = gr.Slider(0.0, 1.0, value=0.65, label="Proporción de mezcla imagen Original/Mejorada")
                with gr.Row():
                    with gr.Accordion(label="Enmascarado", open=False):
                        with gr.Row():
                            with gr.Column(scale=1):
                                chk_useclip = gr.Checkbox(label="Usar Enmascarado de Texto", value=False)
                                clip_text = gr.Textbox(label="Lista de objetos a enmascarar y restaurar en imagen falsa", placeholder="taza,manos,cabello,banana")
                                gr.Dropdown(["Clip2Seg"], value="Clip2Seg", label="Motor")
                            with gr.Column(scale=1):
                                bt_preview_mask = gr.Button("Mostrar Vista Previa de Máscara", variant='secondary')
                            with gr.Column(scale=2):
                                maskpreview = gr.Image(label="Vista Previa de Máscara", shape=(None,512), interactive=False)
                            
                with gr.Row(variant='panel'):
                    with gr.Column():
                        bt_start = gr.Button("Iniciar", variant='primary')
                    with gr.Column():
                        bt_stop = gr.Button("Detener", variant='secondary')
                    with gr.Column():
                        fake_preview = gr.Checkbox(label="Intercambiar caras en frames", value=False)
                    with gr.Column():
                        bt_refresh_preview = gr.Button("Actualizar", variant='secondary')
                with gr.Row(variant='panel'):
                    with gr.Column():
                        with gr.Accordion(label="Resultados", open=True):
                            gr.Button("Abrir Carpeta de Salida", size='sm').click(fn=lambda: util.open_folder(util.resolve_relative_path('../output/')))
                            resultfiles = gr.Files(label='Archivo(s) Procesado(s)', interactive=False)
                            resultimage = gr.Image(type='filepath', interactive=False)
                    with gr.Column():
                        with gr.Accordion(label="Vista Previa Frame Original/Falso", open=True):
                            previewimage = gr.Image(label="Imagen de Vista Previa", interactive=False)
                            with gr.Row(variant='panel'):
                                with gr.Column():
                                    preview_frame_num = gr.Slider(0, 0, value=0, label="Número de Frame", step=1.0, interactive=True)
                                with gr.Column():
                                    bt_use_face_from_preview = gr.Button("Usar Cara de este Frame", variant='primary')
                        
            with gr.Tab("Cámara en Vivo"):
                cam_toggle = gr.Checkbox(label='Activar', value=live_cam_active)
                if live_cam_active:
                    with gr.Row():
                        with gr.Column():
                            cam = gr.Webcam(label='Cámara', source='webcam', mirror_webcam=True, interactive=True, streaming=False)
                        with gr.Column():
                            fake_cam_image = gr.Image(label='Salida de Cámara Falsa', interactive=False)


            with gr.Tab("Extras"):
                with gr.Row():
                    files_to_process = gr.Files(label='Archivo(s) a procesar', file_count="multiple")
                # with gr.Row(variant='panel'):
                    ...
                    with gr.Accordion(label="Video/GIF", open=False):
                        with gr.Row(variant='panel'):
                            with gr.Column():
                                gr.Markdown("""
                                            # Cortar video
                                            Ten en cuenta que esto significa re-codificar el video lo cual puede tomar más tiempo.
                                            La codificación usa tu configuración de la pestaña Configuración.
    """)
                            with gr.Column():
                                cut_start_time = gr.Slider(0, 1000000, value=0, label="Frame Inicial", step=1.0, interactive=True)
                            with gr.Column():
                                cut_end_time = gr.Slider(1, 1000000, value=1, label="Frame Final", step=1.0, interactive=True)
                            with gr.Column():
                                start_cut_video = gr.Button("Iniciar")
    ...
    #                             start_join_videos = gr.Button("Start")
                        with gr.Row(variant='panel'):
                            gr.Markdown("Extraer frames del video")
                            start_extract_frames = gr.Button("Iniciar")
                        with gr.Row(variant='panel'):
                            gr.Markdown("Crear video desde archivos de imagen")
                            gr.Button("Iniciar").click(fn=lambda: gr.Info('Aún no implementado...'))
                        with gr.Row(variant='panel'):
                            gr.Markdown("Crear GIF desde video")
                            start_create_gif = gr.Button("Crear GIF")
                with gr.Row():
                    extra_files_output = gr.Files(label='Archivos de salida resultantes', file_count="multiple")
                        
            
            with gr.Tab("Configuración"):
                with gr.Row():
                    with gr.Column():
                        themes = gr.Dropdown(available_themes, label="Tema", info="El cambio requiere reinicio completo", value=roop.globals.CFG.selected_theme)
                    with gr.Column():
                        settings_controls.append(gr.Checkbox(label="Servidor Público", value=roop.globals.CFG.server_share, elem_id='server_share', interactive=True))
                        settings_controls.append(gr.Checkbox(label='Limpiar carpeta de salida antes de cada ejecución', value=roop.globals.CFG.clear_output, elem_id='clear_output', interactive=True))
                    with gr.Column():
                        input_server_name = gr.Textbox(label="Nombre del Servidor", lines=1, info="Dejar en blanco para ejecutar localmente", value=roop.globals.CFG.server_name)
                    with gr.Column():
                        input_server_port = gr.Number(label="Puerto del Servidor", precision=0, info="Dejar en 0 para usar por defecto", value=roop.globals.CFG.server_port)
                with gr.Row():
                    with gr.Column():
                        settings_controls.append(gr.Dropdown(providerlist, label="Proveedor", value=roop.globals.CFG.provider, elem_id='provider', interactive=True))
                        settings_controls.append(gr.Checkbox(label="Forzar CPU para Analizador de Caras", value=roop.globals.CFG.force_cpu, elem_id='force_cpu', interactive=True))
                        max_threads = gr.Slider(1, 64, value=roop.globals.CFG.max_threads, label="Máx. Número de Hilos", info='por defecto: 8', step=1.0, interactive=True)
                    with gr.Column():
                        memory_limit = gr.Slider(0, 128, value=roop.globals.CFG.memory_limit, label="Máx. Memoria a usar (Gb)", info='0 significa sin límite', step=1.0, interactive=True)
                        frame_buffer_size = gr.Slider(1, 512, value=roop.globals.CFG.frame_buffer_size, label="Tamaño del Buffer de Frames", info='Núm. de Imágenes a precargar por hilo', step=1.0, interactive=True)
                        settings_controls.append(gr.Dropdown(image_formats, label="Formato de Salida de Imagen", info='por defecto: png', value=roop.globals.CFG.output_image_format, elem_id='output_image_format', interactive=True))
                    with gr.Column():
                        settings_controls.append(gr.Dropdown(video_codecs, label="Códec de Video", info='por defecto: libx264', value=roop.globals.CFG.output_video_codec, elem_id='output_video_codec', interactive=True))
                        settings_controls.append(gr.Dropdown(video_formats, label="Formato de Salida de Video", info='por defecto: mp4', value=roop.globals.CFG.output_video_format, elem_id='output_video_format', interactive=True))
                        video_quality = gr.Slider(0, 100, value=roop.globals.CFG.video_quality, label="Calidad de Video (crf)", info='por defecto: 14', step=1.0, interactive=True)
                    with gr.Column():
                        button_apply_restart = gr.Button("Reiniciar Servidor", variant='primary')
                        settings_controls.append(gr.Checkbox(label='Iniciar con cámara en vivo activa', value=roop.globals.CFG.live_cam_start_active, elem_id='live_cam_start_active', interactive=True))
                        button_clean_temp = gr.Button("Limpiar carpeta temporal")
                        button_apply_settings = gr.Button("Aplicar Configuración")
...
def on_option_changed(evt: gr.SelectData):
    ...
    if isinstance(evt.target, gr.Checkbox):
        ...
        if hasattr(roop.globals, attribname):
            ...
            return
    raise gr.Error(f'Configuración no manejada para {evt.target}')
...
def on_srcimg_changed(imgsrc, progress=gr.Progress()):
    ...
    progress(1.0, desc="Obteniendo caras de la imagen")      
    if len(thumbs) < 1:
        raise gr.Error('¡No se detectaron caras!')
...
def on_use_face_from_selected(files, frame_num):
    ...
    if util.is_image(roop.globals.target_path) and not roop.globals.target_path.lower().endswith(('gif')):
        ...
        if len(SELECTION_FACES_DATA) > 0:
            ...
            gr.Info('¡No se detectaron caras!')
...
def start_swap( enhancer, detection, keep_fps, keep_frames, skip_audio, face_distance, blend_ratio,
    ...
    if roop.globals.face_swap_mode == 'selected':
        if len(roop.globals.TARGET_FACES) < 1:
            gr.Error('¡No se seleccionó cara objetivo!')
            return gr.Button.update(variant="primary"),None, None
...
def stop_swap():
    ...
    gr.Info('Abortando procesamiento - por favor espera a que se detengan los hilos restantes')
...
def on_cam_toggle(state):
    ...
    gr.Warning('¡El servidor será reiniciado para este cambio!')
    restart_server = True
...
def on_cut_video(files, cut_start_frame, cut_end_frame):
    ...
    for tf in files:
        ...
        if os.path.isfile(destfile):
            ...
            gr.Error('¡Falló el corte del video!')
    return resultfiles
...
def on_join_videos(files):
    ...
    if os.path.isfile(destfile):
        ...
        gr.Error('¡Falló la unión de videos!')
    return resultfiles
...
def apply_settings(themes, input_server_name, input_server_port):
    ...
    show_msg('Configuración guardada')
