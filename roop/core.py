...
def parse_args() -> None:
    ...
    program.add_argument('-s', '--source', help='seleccionar una imagen origen', dest='source_path')
    ...
    program.add_argument('-f', '--folder', help='seleccionar carpeta objetivo con imágenes o videos para procesar en lote', dest='target_folder_path')
    program.add_argument('--frame-processor', help='procesadores de frames (opciones: face_swapper, face_enhancer, ...)', dest='frame_processor', default=['face_swapper'], nargs='+')
    program.add_argument('--keep-fps', help='mantener fps objetivo', dest='keep_fps', action='store_true')
    program.add_argument('--keep-frames', help='mantener frames temporales', dest='keep_frames', action='store_true')
    program.add_argument('--skip-audio', help='omitir audio objetivo', dest='skip_audio', action='store_true')
    program.add_argument('--many-faces', help='procesar todas las caras', dest='many_faces', action='store_true')
    program.add_argument('--source-face_index', help='posición índice de cara origen en imagen', dest='source_face_index', type=int, default=0)
    program.add_argument('--target-face_index', help='posición índice de cara objetivo en imagen', dest='target_face_index', type=int, default=0)
    program.add_argument('--video-encoder', help='ajustar codificador de video de salida', dest='video_encoder', default='libx264', choices=['libx264', 'libx265', 'libvpx-vp9'])
    program.add_argument('--video-quality', help='ajustar calidad de video de salida', dest='video_quality', type=int, default=18, choices=range(52), metavar='[0-51]')
    program.add_argument('--max-memory', help='cantidad máxima de RAM en GB', dest='max_memory', type=int, default=suggest_max_memory())
    program.add_argument('--execution-provider', help='proveedor de ejecución disponible (opciones: cpu, ...)', dest='execution_provider', default=['cpu'], choices=suggest_execution_providers(), nargs='+')
